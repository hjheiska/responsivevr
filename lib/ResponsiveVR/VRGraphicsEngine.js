...
(function (VRGraphicsEngine, THREE, screen) {
	...
	var animate = function() {
		...
		if(avatar.model.children.length > 0) {
			...
			for(var i = 0; i < avatar.bones.length; i++) {
				...
				switch(avatar.bones[i].name) {
					...
					case "rightArm":
					case "leftArm":
						// Left and right arm for debug
						setJoinRotation(avatar.bones[i], sceneModel.state.inputDevices.local.HMDs[0]);
					break;
					default:
						setJoinRotation(avatar.bones[i], {x :10, y:0, z:0, w:1 } );
					break;
				}
			}
		}
		...
		if(sceneModel.state.logicUpdate || state.newStateReceived) {
			checkVisibility();
			sceneModel.state.logicUpdate = false;
			state.newStateReceived = false;
		}
