...
(function (VRGraphicsEngine, THREE, screen) {
	...
	var setBoneOffsets = function(bone) {
		...
		/*
		bone.meshGroup.rotation.set(
			bone.pivotRotationOffset.x,
			bone.pivotRotationOffset.y,
			bone.pivotRotationOffset.z,
			'XYZ'
		);
		*/
	}
	...
	var animate = function() {
		...
		if(avatar.model.children.length > 0) {
			if(avatar.bones.head.meshGroup == null) {
				avatar.bones.head.meshGroup = avatar.model.getObjectByName(avatar.boneNames.head);
				setBoneOffsets(avatar.bones.head);
			}
			else {
				if(sceneModel.state.inputDevices.local.HMDs[0]) {
					var offsetRotation = new THREE.Quaternion();
					offsetRotation.setFromEuler(
						new THREE.Euler(
							avatar.bones.head.pivotRotationOffset.x,
							avatar.bones.head.pivotRotationOffset.y,
							avatar.bones.head.pivotRotationOffset.z,
							'XYZ'
						)
					);
					var hmdRotation = new THREE.Quaternion(
						sceneModel.state.inputDevices.local.HMDs[0].x,
						sceneModel.state.inputDevices.local.HMDs[0].y,
						sceneModel.state.inputDevices.local.HMDs[0].z,
						sceneModel.state.inputDevices.local.HMDs[0].w
					);
					offsetRotation.multiply(hmdRotation);
					avatar.bones.head.meshGroup.rotation.setFromQuaternion(offsetRotation);
				}
			}
		}
