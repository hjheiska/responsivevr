...
(function (VRGraphicsEngine, THREE, screen) {
	...
	VRGraphicsEngine.connectTo = function(model) {
		...
		newScene.add(backSphere, toggleHudSphere);
		...
		setObjectPositionAndRotationOnCircle(contentPlane, 0, 3, 0, 0, 1);
	...
	var hideShowObjectAndContents = function(object, showObject, showContents, currentContentIndex, currentArticleIndex) {
		...
		if(object.vr.type == "article" && object.vr.index == currentArticleIndex) {
				if(sceneModel.state.logicUpdate) activateArticle(object.vr.element);
			}
			...
			for(j = 0; j < contents.children.length; j++) {
				if(showContents && (currentContentIndex == -1 || currentContentIndex == j)) {
					if(currentContentIndex != -1 && contents.children[j].vr.type == "content") {
						if(sceneModel.state.logicUpdate) activateContent(contents.children[j].vr.element);
	...
	var activateContent = function(content) {
		...
		var contentPlaneWidth = 1.5;
		var contentPlaneWidthInPixels = parseInt((contentPlaneWidth / TOTAL_HUD_WIDTH) * HUD_WIDTH_IN_PIXELS);
		
		var contentPlaneMesh = contentPlane.getObjectByName("mesh");
		if(contentPlaneMesh) contentPlane.remove(contentPlaneMesh);
		// Add content plane
		contentPlane.vr = {};
		contentPlane.vr.element = new VRSceneModel.Content(content.text);
		
		var imageSource = "";
		if(typeof content.foregroundElement != 'undefined') {
			if(content.foregroundElement.image != null) {
				 imageSource = content.foregroundElement.image.source;
			}
		}
		/*
		if(imageSource) {
			contentPlane.vr.element.text = '<div style="font-size: 18px; padding: 10px; max-width: ' + contentPlaneWidthInPixels + 'px"><img style="max-width: 20%; float: left; padding-right: 10px; padding-bottom: 10px;" src="' + imageSource + '" />' + contentPlane.vr.element.text + '</body>';
		}
		else {
		*/
			contentPlane.vr.element.text = '<div style="-moz-column-count: 2; -webkit-column-count: 2; font-size: 18px; padding: 10px; box-sizing: border-box; width: ' + contentPlaneWidthInPixels + 'px">' + contentPlane.vr.element.text + '</div>';
		//}
		...
		var visualAngle = getVisualAngle(contentPlaneWidth);
		...
		loadAndAddmesh(geometry, contentPlane, contentPlaneWidth, contentPlaneHeight);
	...
	var loadAndAddmesh = function(geometry, Object3D, elementWidth, elementHeight) {
		...
		if(imageSource) { 
			if(contentElement) menuText = "<img style='max-width:" + width + "px; max-height:" + height + "px' src='" +  imageSource + "' />";
			else  menuText = "<img style='max-width:" + width + "px; max-height:" + (height - 40) + "px' src='" +  imageSource + "' />" + menuText;
		}
		var html = '<html><body style="margin: 0; padding: 0;"><div style="' + style + '">' + menuText + '</div></body></html>';
		//var dimensions = getMarkupRenderedSize(html);
