...
function VRSceneModel() {
	...
	*/
	...
	this.addArticle = function(title, contents, foregroundElementData, backgroundElementData) { 
		...
		for(var i = 0; i < contents.length; i++) {
			var newContent = new VRSceneModel.Content(contents[i].text, contents[i].foregroundElementData, contents[i].backgroundElementData);
	...
	this.addHeader = function(text) {
		...
		this.state.elements.header = newHeader;
	}
	...
	*/
	var checkLogic = function() {
		if(state.navigation.selections.linkSelected != null) {
			...
			if(!changingScene && RemoteConnection.connectionIsOpen()) state.newStateToSend = true;
			if(!changingScene && !state.newStateToSend) getNewModel(href);
			changingScene = true;
		}
	...
	var getNewModel = function(href) {
		Utils.ajaxRequest("GET", href, null, function(response) {
			ResponsiveVR.reInit(response);
		});
	}
	...
	*/
	...
	VRSceneModel.Content = function(text, foregroundElementData, backgroundElementData) {
		this.text = text;
		this.foregroundElement = new AssociatedElement(foregroundElementData);
		this.backgroundElement = new AssociatedElement(backgroundElementData);
	}
	...
	function Object(source, scale, position, rotation) {
		this.source = source;
