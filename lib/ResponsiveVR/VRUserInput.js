...
(function (VRUserInput) {
	...
	var qrTrackerLoop = function() {
		...
		document.getElementById("debug").innerHTML = threshold;
	...
	var getQRTracking = function(callback) {
		...
		navigator.getUserMedia({'video': true},
		  function(stream) {
				...
				ARCanvas.getContext('2d').translate(ARCanvas.width * 0.5, ARCanvas.height * 0.5);
				ARCanvas.getContext('2d').rotate(Math.PI/2);
				ARCanvas.getContext('2d').translate(-ARCanvas.height * 0.5, -ARCanvas.width * 0.5);
				...
				ctx = ARCanvas.getContext('2d'); // Needed for the library
				ARRaster = new NyARRgbRaster_Canvas2D(ARCanvas); // create reader for the video canvas
				var param = new FLARParam(ARCanvas.width,ARCanvas.height); // create new Param for the canvas [~camera params]
				console.log(param);
				state.inputDevices.local.webcamCameraParams = param;
				ARResultMat = new NyARTransMatResult(); // store matrices we get in this temp matrix
				ARTracker = new FLARMultiIdMarkerDetector(param, 0.07); // marker size is 80 [transform matrix units]
